...
func resourceUCloudBareMetalInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating bare metal instance, %s", err)
	}
...
func resourceUCloudBareMetalInstanceRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading bare metal instance, %s", err)
	}
	conn := client.uphostconn
...
func resourceUCloudBareMetalInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when updating bare metal instance, %s", err)
	}
	conn := client.uphostconn

	updateFunc := func(updateAction func() error) error {
		err := stopUpdateStartBareMetalInstance(d, client, updateAction)
		if err != nil {
			return err
		}
		return nil
...
func resourceUCloudBareMetalInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting bare metal instance, %s", err)
	}
	conn := client.uphostconn
...
func bareMetalInstanceStateRefreshFunc(client *productClient, instanceId string) resource.StateRefreshFunc {
...
func stopUpdateStartBareMetalInstance(d *schema.ResourceData, client *productClient, updateFunc func() error) error {
	conn := client.uphostconn
...
func bareMetalInstanceCustomizeDiff(diff *schema.ResourceDiff, v interface{}) error {
	client, err := clientFromMeta(v)
	if err != nil {
		return fmt.Errorf("error on getting client when validating bare metal instance, %s", err)
	}
	conn := client.uphostconn
