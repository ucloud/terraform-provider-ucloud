...
func testAccCheckDiskAttachmentExists(n string, diskSet *udisk.UDiskDataSet, instance *uhost.UHostInstanceSet) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		return resource.Retry(3*time.Minute, func() *resource.RetryError {
			d, found, err := describeAccDiskResource(clients.udisk, diskId, resourceId)

			if err != nil {
				return resource.NonRetryableError(err)
			}

			if found && d.UDiskId == diskSet.UDiskId && d.UHostId == instance.UHostId {
				return nil
			}
...
func testAccCheckDiskAttachmentDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		...
		d, found, err := describeAccDiskResource(clients.udisk, diskID, instanceID)
		if err != nil {
			return err
		}
		if !found {
