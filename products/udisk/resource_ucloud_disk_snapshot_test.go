...
func testAccCheckDiskSnapshotExists(n string, snapshotSet *udisk.UDiskSnapshotSet) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		ptr, found, err := describeAccDiskSnapshotByID(clients.udisk, rs.Primary.ID)
		...
		if !found {
			return fmt.Errorf("disk snapshot %q is not found", rs.Primary.ID)
		}
...
func testAccCheckDiskSnapshotDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		d, found, err := describeAccDiskSnapshotByID(clients.udisk, rs.Primary.ID)
		if err != nil {
			return err
		}
		if !found {
