...
func resourceUCloudDiskCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating disk, %s", err)
	}
...
func resourceUCloudDiskCreateFromSnapshot(d *schema.ResourceData, meta interface{}, snapshotId string) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when creating disk from snapshot, %s", err)
	}
	conn := client.udiskconn
...
func resourceUCloudDiskUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when updating disk, %s", err)
	}
	conn := client.udiskconn
...
func resourceUCloudDiskRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading disk, %s", err)
	}
...
func resourceUCloudDiskDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting disk, %s", err)
	}
	conn := client.udiskconn
...
func diskWaitForState(client *productClient, diskId string) *resource.StateChangeConf {
