...
func resourceUCloudDiskAttachmentCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating disk attachment, %s", err)
	}
...
func resourceUCloudDiskAttachmentRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading disk attachment, %s", err)
	}
...
func resourceUCloudDiskAttachmentDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting disk attachment, %s", err)
	}
	conn := client.udiskconn
	...
	if _, ok := d.GetOk("stop_instance_before_detaching"); ok {
		err := waitAndUpdateInstanceState(client, *req.HostId, statusStopped, d.Timeout(schema.TimeoutDelete))
		if err != nil {
...
func diskAttachmentStateRefreshFunc(client *productClient, diskId string) resource.StateRefreshFunc {
