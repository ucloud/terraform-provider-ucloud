...
func testAccCheckDiskExists(n string, diskSet *udisk.UDiskDataSet) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		ptr, found, err := describeAccDiskByID(clients.udisk, rs.Primary.ID)
		...
		if !found {
			return fmt.Errorf("disk %q is not found", rs.Primary.ID)
		}
...
func testAccCheckDiskDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		d, found, err := describeAccDiskByID(clients.udisk, rs.Primary.ID)
		if err != nil {
			return err
		}
		if !found {
