...
func resourceUCloudDiskSnapshotCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating disk snapshot, %s", err)
	}
...
func resourceUCloudDiskSnapshotRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading disk snapshot, %s", err)
	}
...
func resourceUCloudDiskSnapshotDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting disk snapshot, %s", err)
	}
	conn := client.udiskconn
...
func diskSnapshotWaitForState(client *productClient, snapshotId string) *resource.StateChangeConf {
