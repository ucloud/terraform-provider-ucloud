...
func testAccCheckUFSVolumeMountPointExists(n string, ufsSet *ufs.MountPointInfo) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		ptr, found, err := describeAccUFSMountPoint(client, volumeId, vpcId, subnetId)
		...
		if !found {
			return fmt.Errorf("ufs volume mount point %q is not found", rs.Primary.ID)
		}
...
func testAccCheckUFSVolumeMountPointDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		...
		vpcId := rs.Primary.Attributes["vpc_id"]
		subnetId := rs.Primary.Attributes["subnet_id"]
		d, found, err := describeAccUFSMountPoint(client, volumeId, vpcId, subnetId)
