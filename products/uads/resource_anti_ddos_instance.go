...
func resourceUCloudAntiDDoSInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	uadsServiceInfo, err := describeUADSById(client, d.Id())
...
func resourceUCloudAntiDDoSInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeUADSById(client, d.Id())
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
...
func antiDDoSInstanceWaitForState(client *sdkuads.UADSClient, id string) *resource.StateChangeConf {
	return &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			uadsInfo, err := describeUADSById(client, id)
			if err != nil {
				if isNotFoundError(err) {
