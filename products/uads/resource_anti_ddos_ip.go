...
func resourceUCloudAntiDDoSIPUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("comment") && !d.IsNewResource() {
		...
		if err != nil {
			return fmt.Errorf("fail to set comment for ip %v of %v", comment, ip)
		}
...
func resourceUCloudAntiDDoSIPRead(d *schema.ResourceData, meta interface{}) error {
	...
	instanceId := items[0]
	ip := items[1]
	_, err = describeUADSById(client, instanceId)
	if err != nil {
	...
	ipInfo, err := describeUADSBGPServiceIP(client, instanceId, ip)
...
func resourceUCloudAntiDDoSIPDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeUADSBGPServiceIP(client, instanceId, ip)
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
...
func antiDDoSIPWaitForState(client *sdkuads.UADSClient, id string, ip string) *resource.StateChangeConf {
	return &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			ipInfo, err := describeUADSBGPServiceIP(client, id, ip)
			if err != nil {
				log.Fatal(err)
				if isNotFoundError(err) {
