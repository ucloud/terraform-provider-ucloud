...
func resourceUCloudAntiDDoSAllowedDomainRead(d *schema.ResourceData, meta interface{}) error {
	...
	domainInfo, err := describeUADSAllowedDomain(client, instanceId, domain)
...
func resourceUCloudAntiDDoSAllowedDomainDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeUADSAllowedDomain(client, instanceId, domain)
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
...
func antiDDoSAllowedDomainWaitForState(client *sdkuads.UADSClient, id string, domain string) *resource.StateChangeConf {
	return &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			domainInfo, err := describeUADSAllowedDomain(client, id, domain)
			if err != nil {
				if isNotFoundError(err) {
