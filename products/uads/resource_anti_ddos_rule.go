...
func resourceUCloudAntiDDoSRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	ruleInfo, err := describeUADSBGPServiceFwdRuleByIpPort(client, instanceId, ip, port)
	if err != nil {
...
func resourceUCloudAntiDDoSRuleDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeUADSBGPServiceFwdRule(client, instanceId, ruleIndex)
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
...
func antiDDoSRuleWaitForState(client *sdkuads.UADSClient, id string, ruleIndex int) *resource.StateChangeConf {
	return &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			ruleInfo, err := describeUADSBGPServiceFwdRule(client, id, ruleIndex)
			if err != nil {
				if isNotFoundError(err) {
