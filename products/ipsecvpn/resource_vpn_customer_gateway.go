...
func resourceUCloudVPNCustomerGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating vpn customer gateway, %s", err)
	}
	conn := client
...
func resourceUCloudVPNCustomerGatewayRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading vpn costomer gateway, %s", err)
	}
	vgSet, err := describeVPNCustomerGatewayByID(client, d.Id())
...
func resourceUCloudVPNCustomerGatewayDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting vpn costomer gateway, %s", err)
	}
	conn := client
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeVPNCustomerGatewayByID(client, d.Id())
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
