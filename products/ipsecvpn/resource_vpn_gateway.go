...
func resourceUCloudVPNGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating vpn gateway, %s", err)
	}
	conn := client
...
func resourceUCloudVPNGatewayUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when updating vpn gateway, %s", err)
	}
	conn := client
...
func resourceUCloudVPNGatewayRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading vpn gateway, %s", err)
	}
	vgSet, err := describeVPNGatewayByID(client, d.Id())
...
func resourceUCloudVPNGatewayDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting vpn gateway, %s", err)
	}
	conn := client
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeVPNGatewayByID(client, d.Id())
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
