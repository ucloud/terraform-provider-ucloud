...
func resourceUCloudVPNConnectionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating vpn connection, %s", err)
	}
	conn := client
...
func resourceUCloudVPNConnectionUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when updating vpn connection, %s", err)
	}
	conn := client
	d.Partial(true)
...
func resourceUCloudVPNConnectionRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading vpn connection, %s", err)
	}
	vcSet, err := describeVPNConnectionByID(client, d.Id())
...
func resourceUCloudVPNConnectionDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting vpn connection, %s", err)
	}
	conn := client
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeVPNConnectionByID(client, d.Id())
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
