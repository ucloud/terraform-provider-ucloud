...
func dataSourceUCloudIAMPolicyDocumentRead(d *schema.ResourceData, meta interface{}) error {
	if value, ok := d.GetOk("statement"); ok {
		document, err := assembleDataSourcePolicyJSON(value.([]interface{}), d.Get("version").(string))
		if err != nil {
			return err
		}
		if output, ok := d.GetOk("output_file"); ok && output.(string) != "" {
			_ = writeToFile(output.(string), document)
		}
		_ = d.Set("json", document)
		d.SetId(hashString(document))
	}
	return nil
}
...
func assembleDataSourcePolicyJSON(statements []interface{}, version string) (string, error) {
	...
	for _, value := range statements {
		statementMap := value.(map[string]interface{})
		statement := PolicyStatement{Effect: statementMap["effect"].(string)}
