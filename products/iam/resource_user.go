...
func resourceUCloudIAMUserCreate(d *schema.ResourceData, meta interface{}) error {
	...
	req := client.iamconn.NewCreateUserRequest()
	...
	if value, ok := d.GetOk("display_name"); ok {
		req.DisplayName = ucloud.String(value.(string))
	}
	if d.Get("login_enable").(bool) {
		if value, ok := d.GetOk("email"); ok {
			req.Email = ucloud.String(value.(string))
		} else {
	...
	resp, err := client.iamconn.CreateUser(req)
	if err != nil {
	...
	if d.Get("is_frozen").(bool) {
		updateReq := client.iamconn.NewUpdateUserRequest()
		updateReq.UserName = ucloud.String(d.Get("name").(string))
		updateReq.Status = ucloud.String(iamStatusFrozen)
		if _, err = client.iamconn.UpdateUser(updateReq); err != nil {
...
func resourceUCloudIAMUserUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	profileReq := client.iamconn.NewUpdateLoginProfileRequest()
	...
	if d.HasChanges("display_name", "is_frozen") {
		...
		if _, err := client.iamconn.UpdateUser(req); err != nil {
...
func resourceUCloudIAMUserRead(d *schema.ResourceData, meta interface{}) error {
	...
	_ = d.Set("is_frozen", user.Status == iamStatusFrozen)
	_ = d.Set("login_enable", loginProfile.Status == iamStatusActive)
	return nil
}
...
func resourceUCloudIAMUserDelete(d *schema.ResourceData, meta interface{}) error {
	...
	req := client.iamconn.NewDeleteUserRequest()
	req.UserName = ucloud.String(d.Id())

	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		if _, err := client.iamconn.DeleteUser(req); err != nil {
