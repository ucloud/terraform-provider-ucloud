...
func dataSourceUCloudProjectsRead(d *schema.ResourceData, meta interface{}) error {
	conn, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading project list, %s", err)
	}

	req := conn.NewGetProjectListRequest()

	if value, ok := d.GetOk("is_finance"); ok {
		req.IsFinance = ucloud.String(boolLowerCvt.convert(value.(bool)))
	}
	...
	var projects []sdkuaccount.ProjectListInfo
	...
	if err := dataSourceUCloudProjectsSave(d, projects); err != nil {
		return fmt.Errorf("error on reading project list, %s", err)
	}
...
func dataSourceUCloudProjectsSave(d *schema.ResourceData, projects []sdkuaccount.ProjectListInfo) error {
	ids := []string{}
