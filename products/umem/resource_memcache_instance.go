...
func resourceUCloudMemcacheInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating memcache instance, %s", err)
	}
...
func resourceUCloudMemcacheInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when updating memcache instance, %s", err)
	}
...
func resourceUCloudMemcacheInstanceRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading memcache instance, %s", err)
	}
...
func resourceUCloudMemcacheInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting memcache instance, %s", err)
	}
	conn := client.umemconn
...
func (c *productClient) waitActiveStandbyMemcacheRunning(id string) error {
