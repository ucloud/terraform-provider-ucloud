...
func resourceUCloudEIPAssociationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting unet client when creating eip association, %s", err)
	}
	conn := client
...
func resourceUCloudEIPAssociationRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when reading eip association, %s", err)
	}

	p := strings.Split(d.Id(), ":")
	res, err := describeEIPResourceById(client, p[0], p[1])
	if err != nil {
...
func resourceUCloudEIPAssociationDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when deleting eip association, %s", err)
	}
	conn := client
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		_, err := describeEIPResourceById(client, p[0], p[1])
		if err != nil {
			if isNotFoundError(err) {
				d.SetId("")
				return nil
