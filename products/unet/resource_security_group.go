...
func resourceUCloudSecurityGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting unet client when creating security group, %s", err)
	}
	conn := client
...
func resourceUCloudSecurityGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when updating security group, %s", err)
	}
	conn := client
...
func resourceUCloudSecurityGroupRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when reading security group, %s", err)
	}
	sgSet, err := describeFirewallById(client, d.Id())
...
func resourceUCloudSecurityGroupDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when deleting security group, %s", err)
	}
	conn := client
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeFirewallById(client, d.Id())
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
...
func securityWaitForState(client *sdkClient, sgId string) *resource.StateChangeConf {
	return &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			sgSet, err := describeFirewallById(client, sgId)
			if err != nil {
				if isNotFoundError(err) {
