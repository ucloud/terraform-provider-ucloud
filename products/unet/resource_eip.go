...
func resourceUCloudEIPCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting unet client when creating eip, %s", err)
	}
	conn := client
...
func resourceUCloudEIPUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when updating eip, %s", err)
	}
	conn := client
...
func resourceUCloudEIPRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when reading eip, %s", err)
	}

	eip, err := describeEIPById(client, d.Id())
	if err != nil {
...
func resourceUCloudEIPDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting unet client when deleting eip, %s", err)
	}
	conn := client
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		_, err := describeEIPById(client, d.Id())
		if err != nil {
			if isNotFoundError(err) {
				return nil
			}
...
func eipWaitForState(client *sdkClient, eipId string) *resource.StateChangeConf {
	return &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			eip, err := describeEIPById(client, eipId)
			if err != nil {
				if isNotFoundError(err) {
