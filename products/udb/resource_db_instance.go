...
func resourceUCloudDBInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error on getting client when creating db instance, %s", err)
	}
	conn := client
...
func resourceUCloudDBInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when updating db instance, %s", err)
	}
	conn := client
	...
	if d.HasChange("parameter_group") && !d.IsNewResource() {
		dbInstance, err := describeDBInstanceByID(client, d.Id())
		if err != nil {
...
func resourceUCloudDBInstanceRead(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when reading db instance, %s", err)
	}

	db, err := describeDBInstanceByID(client, d.Id())
	if err != nil {
		if isNotFoundError(err) {
			d.SetId("")
			return nil
...
func resourceUCloudDBInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client, err := clientFromMeta(meta)
	if err != nil {
		return fmt.Errorf("error on getting client when deleting db instance, %s", err)
	}
	conn := client
	...
	return resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		if _, err := describeDBInstanceByID(client, d.Id()); err != nil {
			if isNotFoundError(err) {
				return nil
			}
...
func dbInstanceStateRefreshFunc(client *udb.UDBClient, dbId string, target []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		db, err := describeDBInstanceByID(client, dbId)
		if err != nil {
			if isNotFoundError(err) {
