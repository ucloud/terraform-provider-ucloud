...
* `charge_mode` -(Optional) Elastic IP charge mode. Possible values are: `traffic` as pay by traffic, `bandwidth` as pay by bandwidth, `share_bandwidth` as binding to a shared bandwidth package. (Default: `bandwidth`for the Elastic IP).
