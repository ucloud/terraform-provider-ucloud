...
* `type` - (Required, ForceNew) The type of the cloud data disk. Possible values are: `cloud_normal` for cloud disk, `cloud_ssd` for SSD cloud disk, `cloud_essd` for economy SSD cloud disk, `cloud_rssd` as RDMA-SSD cloud disk.
